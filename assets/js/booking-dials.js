...
export function mountBookingDials(root,{t}) {
 ...
 ui.querySelectorAll('[data-dial]').forEach(dial=>{const id=dial.dataset.dial;let drag=null;const pointers=new Set();
  ...
  dial.addEventListener('pointerdown',e=>{pointers.add(e.pointerId);if(pointers.size>1||!e.isPrimary){drag=null;return;}dial.focus({preventScroll:true});drag={id:e.pointerId,last:angle(e),carry:0};dial.setPointerCapture(e.pointerId);dial.classList.add('is-turning');});
  dial.addEventListener('pointermove',e=>{if(!drag||e.pointerId!==drag.id)return;const a=angle(e);let d=a-drag.last;d=(d+540)%360-180;drag.last=a;drag.carry+=d;const r=range(id),step=360/Math.max(3,r.max-r.min+1);const count=Math.trunc(drag.carry/step);if(count){set(id,value(id)+count);drag.carry-=count*step;}});
